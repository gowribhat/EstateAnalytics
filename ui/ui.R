...
ui <- fluidPage(
  ...
  # Transactions overlay - using simpler display:none approach
  div(
    id = "transactions_overlay",
    class = "transactions-overlay",
    style = "display: none;", # Start hidden with display:none instead of visibility
    div(
      style = "display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;",
      h4("Transaction History", style = "margin: 0;"),
      actionButton("close_transactions", "×", class = "btn btn-sm close-overlay", style = "border-radius: 50%; width: 30px; height: 30px; display: flex; justify-content: center; align-items: center; font-weight: bold;")
    ),
    div(
      style = "height: calc(100% - 40px); overflow-y: auto;",
      DTOutput("building_transactions")
    )
  ),
